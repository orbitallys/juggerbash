...
// - ADDED: player-controlled soul movement during attacks (keyboard/WASD/arrow keys, continuous)
// - ADDED: HP & damage indicators placed next to hp bar visually
// - ADDED: dialogue box is square only while in an attack, rectangular otherwise
...
// -------------------- utility: clearAllProjectiles --------------------
function clearAllProjectiles(){
  // remove DOM elements for projectiles and remove non-text bullets
...
(function ensureHpStyle(){
  ...
  s.innerText = `
    ...
    /* ensure hp indicators can be absolutely positioned without breaking layout */
    #hpCur, #hpMax, #hpLost { position: absolute; pointer-events:none; z-index: 99999; font-family: 'Press Start 2P', monospace; font-size:12px; }
    #hpLost { transition: opacity 300ms ease, width 300ms ease; opacity: 0; background: rgba(220,20,60,0.9); height: 12px; border-radius: 2px; }
  `;
...
async function startGame(){
  ...
  // Show only intro line at battle start
  await setTypedDialogue(JUGGERNAUT_FLAVORS[0]);
  await wait(DEFAULT_DIALOGUE_POST_DELAY);
  
  // Allow player to make choice (not slider/attack)
...
document.addEventListener('keydown', async (e) => {
  ...
  // If the player is in the middle of an attack, allow soul movement via arrows/WASD.
  if (inAttack) {
    // handled by continuous movement loop, but process single-key taps too (so keyboard players get immediate response)
    const moveOnce = (dx, dy) => {
      const rect = dialogueBox ? dialogueBox.getBoundingClientRect() : { left: 360, top: 80, width:260, height:260 };
      const speed = 8;
      // clamp within dialogue battle box bounds
      player.soulX = Math.max(rect.left + 8, Math.min(rect.left + rect.width - 8, player.soulX + dx * speed));
      player.soulY = Math.max(rect.top + 8, Math.min(rect.top + rect.height - 8, player.soulY + dy * speed));
      soul.style.left = (player.soulX - (soul.offsetWidth||24)/2) + 'px';
      soul.style.top = (player.soulY - (soul.offsetHeight||24)/2) + 'px';
    };
    if (['ArrowLeft','a'].includes(e.key)) { moveOnce(-1,0); e.preventDefault(); return; }
    if (['ArrowRight','d'].includes(e.key)) { moveOnce(1,0); e.preventDefault(); return; }
    if (['ArrowUp','w'].includes(e.key)) { moveOnce(0,-1); e.preventDefault(); return; }
    if (['ArrowDown','s'].includes(e.key)) { moveOnce(0,1); e.preventDefault(); return; }
    // while inAttack, ignore other interactions
    return;
  }

  // When not in attack: action navigation and selection
...
function canNavigateActions(){ return playerTurn && !inSlider && !inChoiceMode && !inAttack; }
...
async function performAction(action){
  ...
  if (action === 'fight') await startSlider();
    ...
    // ensure dialogue box back to rectangular during mercy text
    setDialogueBoxSquare(false);
...
function startSlider(){
  return new Promise(res => {
    ...
    // ensure dialogue box not square while using slider (so flavortext etc remain rectangular)
    setDialogueBoxSquare(false);
...
function openActChoices(){
  return new Promise(res => {
    // ensure dialogue box rectangular while selecting acts/flavortext
    setDialogueBoxSquare(false);
    storeAndClearDialog();
...
function openItemChoices(){
  return new Promise(res => {
    // ensure dialogue box rectangular while selecting items/flavortext
    setDialogueBoxSquare(false);
    storeAndClearDialog();
...
async function displayJuggernautFlavor(){
  ...
  // ensure dialogue box rectangular when showing normal flavor (unless it's actually during an attack)
  if (!inAttack) setDialogueBoxSquare(false);
...
function updateBullets(){
  for (let i = bullets.length - 1; i >= 0; i--){
    ...
    if (dist < (b.r + player.size/2)) {
      ...
      if (hpLost) { 
        // position hpLost near hpBar (updateUI handles placement), then show quick damage overlay width
        hpLost.style.width = Math.min(100, (dmg / player.maxHp) * 100) + '%'; 
        hpLost.style.opacity = '0.95'; 
        safeSetTimeout(()=>{ hpLost.style.opacity='0'; hpLost.style.width='0'; },700); 
      }
      updateUI();
      if (player.hp <= 0) {
        stats.deaths += 1;
...
// -------------------- HP indicators placement helper --------------------
function positionHpIndicators(){
  try {
    if (!hpBar || (!hpCur && !hpMax && !hpLost)) return;
    const barRect = hpBar.getBoundingClientRect();
    // place hpCur to the right of hpBar, hpMax to the right of hpCur
    const gap = 8;
    const curW = hpCur ? hpCur.offsetWidth || 48 : 48;
    const maxW = hpMax ? hpMax.offsetWidth || 48 : 48;
    if (hpCur) {
      hpCur.style.left = (barRect.right + gap) + 'px';
      hpCur.style.top = (barRect.top) + 'px';
      hpCur.style.zIndex = 99999;
    }
    if (hpMax) {
      hpMax.style.left = (barRect.right + gap + curW + 6) + 'px';
      hpMax.style.top = (barRect.top) + 'px';
      hpMax.style.zIndex = 99999;
    }
    if (hpLost) {
      // make hpLost overlay appear just above or coincident with the hpBar's right area
      hpLost.style.left = (barRect.left) + 'px';
      hpLost.style.top = (barRect.top) + 'px';
      hpLost.style.width = '0';
      hpLost.style.height = (barRect.height || 12) + 'px';
      // keep zIndex just above the HP bar
      hpLost.style.zIndex = 99998;
    }
  } catch(e){}
}

// updateUI function updated to set hp etc and position indicators
function updateUI(){
  ...
  // position hp indicators near bar
  positionHpIndicators();
}

// -------------------- attack movement loop (continuous movement while inAttack) --------------------
(function attackMovementLoop(){
  const speed = 3.6; // pixels per frame factor (smooth)
  function loop(){
    if (inAttack && dialogueBox && soul) {
      let moved = false;
      let dx = 0, dy = 0;
      ...
      if (dx !== 0 || dy !== 0) {
        ...
        player.soulX = Math.max(rect.left + 8, Math.min(rect.left + rect.width - 8, player.soulX + (dx/mag) * speed));
        player.soulY = Math.max(rect.top + 8, Math.min(rect.top + rect.height - 8, player.soulY + (dy/mag) * speed));
        soul.style.left = (player.soulX - (soul.offsetWidth||24)/2) + 'px';
        soul.style.top = (player.soulY - (soul.offsetHeight||24)/2) + 'px';
        moved = true;
      }
...
(function init(){
  ...
  const row = document.querySelector('.menu-row[data-index="' + menuIndex + '"]');
  if (row) moveSoulToElement(row, true);
  // ensure hp indicators placed correctly on load and on resize
  positionHpIndicators();
  window.addEventListener('resize', positionHpIndicators);
})();

// -------------------- helper: setDialogueBoxSquare --------------------
function setDialogueBoxSquare(makeSquare){
  ...
  if (makeSquare) {
    ...
    // center text top/left within the square (ensure readable)
    if (dialogueText) { dialogueText.style.textAlign = 'center'; dialogueText.style.whiteSpace='pre-wrap'; }
  } else {
    dialogueBox.classList.remove('battle-box');
    dialogueBox.style.width = '';
    dialogueBox.style.height = '';
    dialogueBox.style.background = '#000';
    if (dialogueText) { dialogueText.style.textAlign = ''; dialogueText.style.whiteSpace=''; }
  }
}